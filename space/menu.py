...
imagem_fundo = GameImage("space/bg.png") # GS - Alterei somente onde pega o arquivo
imagem_fundo.set_position(0, 0)

# GS - Carregar imagem nave
nave = Sprite("space/nave.png",frames=1)
...
def tratar_clique_dificuldade(texto_botao): # GS - Adicionei as dificuldades indo diretamente para o jogo e aumentando a dificuldade
    ...
    if texto_botao == "FÁCIL":
        ...
        dif = 3
    return dif
    # Aqui você pode salvar a dificuldade escolhida se quiser
    #estado_atual = MENU
    ...
    # Estado do jogo
        ...
        if nave.x + nave.width <= window.width:
            if keyboard.key_pressed("right"):
                nave.x = nave.x + velNavex * window.delta_time()
        if nave.x >= 0:
            if keyboard.key_pressed("left"):
                nave.x = nave.x + velNavex * window.delta_time() * -1

        if keyboard.key_pressed("space") and tempo_ultimo_tiro >= recarga_tiro:
            tiro = Sprite("space/tiro.png",frames=1)
            tiro.x = nave.x
            tiro.y = nave.y
            tiros.append(tiro)
        ...
        for i in tiros:
            i.draw()
        texto_jogo = "Tela do Jogo - Pressione ESC para voltar"
        largura_texto_jogo = calcular_centro_texto(texto_jogo, 32)
        texto_jogo_x = WINDOW_WIDTH//2 - largura_texto_jogo//2
        window.draw_text(texto_jogo, texto_jogo_x, WINDOW_HEIGHT//2, size=32, color=(255, 255, 255))
